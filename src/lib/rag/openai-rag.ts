...
- البطاقة التي تظهر من العنصر النائب تعرض أصلاً اسم الشريك ووصفه وزر الشراء. لذلك لا تُعد كتابة الوصف في نصّك — تكراره يجعل الجملة نفسها تظهر مرتين على الشاشة. اجعل سطرك يضيف ما لا تستطيع البطاقة قوله: سبب قصير يوضّح لماذا يناسب هذا الشريك تحديداً هذا الطلب ("مناسب لإطلالة سهرة رسمية"، "الأفضل سعراً بين الخيارات الثلاثة"، "الأقرب إلى ميزانيتك"). اذكر اسم الشريك، ثم السبب، ثم ضع العنصر النائب.
- كل نتيجة من الأداة تحتوي على حقل "sells" يصف ما يبيعه ذلك الشريك فعلاً. اقرأه واستخدمه. لا ترشّح إلا نتيجة يغطي حقل "sells" فيها العنصر الذي تتحدث عنه فعلاً، وصِف الشريك بناءً على تلك المعلومات — ولا تخترع أبداً ما يبيعه متجر لتجعله مناسباً للسؤال. الشريك الذي ظهر في بحث عن "قلم" وحقل "sells" لديه يقول إنه يبيع مشالح رجالية تقليدية ليس متجر أقلام، والحقيبة الموصوفة بأنها حقيبة طعام معزولة ليست حقيبة سهرة: تجاهل هذه النتائج بدلاً من تجميلها. من الأفضل دائماً أن تقول إنه لا يوجد لديك خيار مناسب لذلك العنصر بدلاً من تقديم متجر غير ذي صلة وكأنه مناسب.
- إذا لم يُعِد البحث أي نتيجة مناسبة فعلاً، قل بوضوح إنه لا يتوفر لديك حالياً شريك لهذا العنصر تحديداً، وقدّم إرشاداً عاماً موجزاً بدلاً من ذلك. لا تُقحم شريكاً غير ذي صلة لسدّ الفراغ.
...
- The card rendered from a placeholder ALREADY shows the partner's name, its full description and a buy button. Never repeat that description in your own text — it makes the identical sentence appear twice on screen, once from you and once on the card. Write only what the card cannot say: a single short reason this partner fits THIS request, plus a discount code if there is one.
  Bad (repeats the card): "Shantti — Specializes in luxury women's handbags."
  Good (adds something): "Shantti — the most formal of the three, and 5% off with code LA."
  Keep it to one line per partner, then the placeholder.
- Each tool result carries a "sells" field describing what that partner actually offers. Read it and use it. Only recommend a result whose "sells" genuinely covers the item you are discussing, and describe the partner using that information — never invent what a store sells in order to make it fit the question. A partner returned for "a pen" whose "sells" says traditional men's cloaks is NOT a pen shop, and a bag described as an insulated lunch bag is not an evening bag: skip those results rather than dressing them up. It is always better to say you have no match for that item than to present an unrelated store as if it were one.
- If a search returns nothing genuinely suitable, say plainly that you don't currently have a partner for that specific item and give brief general guidance instead. Do not stretch an unrelated partner to fill the gap.
...
export type ToolProductMatch = {
  id: string;
  nameEn: string;
  nameAr: string;
  /** What the partner actually sells. Without this the model only sees a
   *  store name and invents a description to fit the question — which is how
   *  a traditional-cloak retailer got described as "a luxury pen". */
  description?: string;
  subcategory?: string;
};
  ...
  try {
    ...
    for (let iteration = 0; iteration < MAX_TOOL_ITERATIONS; iteration++) {
      ...
      for (const call of toolCalls) {
        ...
        // Hand back what each partner actually sells, so the model can judge
        // whether a result genuinely fits and describe it truthfully instead
        // of guessing from the store name alone.
          ...
          results.length > 0
            ? JSON.stringify(
                results.map((r) => ({
                  ...
                  sells: r.description || r.subcategory || "(no description on file)",
                  category: r.subcategory || undefined,
                })),
              )
            : "NONE_FOUND";
